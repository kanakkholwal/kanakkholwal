...
export function SimpleProjectCards({ cards }: { cards: ProjectType[] }) {
  return (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-7xl mx-auto">
    {cards?.map((card) => (
      <TransitionLink
      ...
      >
        {/* Media Section (Top 65%) */}
        <div className="h-[65%] w-full overflow-hidden relative">
          <div id={`media-${card.title}-${card.id}`} className="h-full w-full">
            {card.video ? (
              <video
                src={card.video}
                autoPlay
        ...
        <div className="flex flex-col justify-between flex-1 p-6 bg-background relative z-10">
          <div>
            ...
            {/* Short Description (Truncated) */}
            <div id={`description-${card.title}-${card.id}`}>
              <p className="mt-2 text-sm text-muted-foreground line-clamp-2">
                {card.description.replace(/[#*_`]/g, '')}
              </p>
            </div>
