      ...
      {selectedStyle === "dynamic" ? (
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-12 lg:gap-24 relative z-10">
            ...
            <div className="sticky top-32 space-y-8">
              ...
              <BlurFade delay={0.3}>
                <div className="flex flex-wrap gap-2 pt-4">
                  {["User Centric", "Pixel Perfect", "Scalable"].map(
                    (tag, i) => (
                      <div
                        key={i}
                        className="px-3 py-1 rounded-full border border-primary/10 bg-primary/5 text-xs font-medium text-primary/80 backdrop-blur-sm"
                      >
                        {tag}
                      </div>
                    ),
          ...
          <div className="md:col-span-8 lg:col-span-8 md:pt-4">
            <RevealTextContent>{appConfig.summary}</RevealTextContent>
...
const RevealTextContent = ({ children }: { children: string }) => {
  ...
  return (
    <div
      ref={ref}
      className="prose prose-lg md:prose-xl dark:prose-invert max-w-none text-muted-foreground leading-relaxed"
    >
      ...
      >
        <Markdown
          components={{
            ...
            ul: ({ children }) => (
              <ul className="space-y-2 mb-6 ml-4 list-none border-l-2 border-primary/20 pl-6">
                {children}
              </ul>
            ),
            li: ({ children }) => (
              <li className="relative pl-2 before:absolute before:left-[-24px] before:top-3 before:h-1.5 before:w-1.5 before:rounded-full before:bg-primary/50">
                {children}
              </li>
            ),
          }}
