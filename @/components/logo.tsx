...
export const Logo = ({
  ...
  const [textWidth, setTextWidth] = useState<number>(2845.44);
  ...
  return (
    <div className={cn("relative flex items-center", containerClassName)}>
      <motion.svg
        ...
        viewBox={`0 0 ${totalWidth} 822.54`}
        className={cn(sizes[size], "w-auto", className)}
        {...props}
      >
        {/* Icon */}
        <motion.path
          variants={draw ? (isLoader ? pathLoaderVariants : pathVariants) : {}}
          initial={draw ? "hidden" : false}
          animate={draw ? (isLoader ? "animate" : "visible") : false}
          stroke="currentColor"
          strokeWidth={10}
          className={cn("fill-foreground", pathClassName, textOnly && "hidden", pathOnly && "block")}
        ...
        <motion.text
          ...
          fontWeight="600"
          fill="currentColor"
          variants={draw ? (isLoader ? textLoaderVariants : textVariants) : {}}
          initial={draw ? "hidden" : false}
          animate={draw ? (isLoader ? "animate" : "visible") : false}
          className={cn(textClassName, pathOnly && "hidden", textOnly && "block")}
        >
