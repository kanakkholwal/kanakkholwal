...
export async function StarHistoryGraph() {
  const stars = await Promise.all(statsConfig.repositories.map(r => r.repo).map(getStarHistory))
  ...
  return (
    <StarsGraph data={starsByRepo} stargazersTab={<StargazersList data={starsByRepo}  key="stars.gazers-list"/>} />
  )
}
