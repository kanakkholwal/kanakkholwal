...
export async function NPMStats() {
  const npmStats = await Promise.all(statsConfig.npmPackages.map(pkg => fetchNpmPackage(pkg)))

  const all = combineStats(
    statsConfig.npmPackages.reduce((acc, pkg, index) => {
      ...
      <DownloadsGraph
        ...
        trend={
          <TrendBadge
            ...
            oldValue={getPartialPreviousWeekDownloads(all.last30Days.reduce((sum, d) => sum.concat(d as unknown as Datum[]), [] as Datum[]))}
        ...
        title={
          <>
            ...
            <dl className="mr-1 ml-auto flex gap-2">
              ...
              <dd>
                {formatStatNumber(
                  all.last90Days.filter((d) => d.downloads).reduce((sum, { downloads }) => sum + Number(downloads), 0)
                )}
              </dd>
              <span className="font-light text-zinc-500" aria-hidden>
                |
              ...
              {statsConfig.npmPackages.map((pkg, index) => {
                return (<Fragment key={pkg}>
                  ...
                  <dt className="sr-only">{pkg}</dt>
                  <dd className="text-zinc-500/50" title={`Last 90 days, ${pkg}`}>
                    {formatStatNumber(
        ...
        title={
          <>
            ...
            <dl className="mr-1 ml-auto flex gap-2">
              ...
              <dd>
                {formatStatNumber(
                  all.last30Days.reduce((sum, { downloads }) => sum + Number(downloads), 0)
                )}
              </dd>
              <span className="font-light text-zinc-500" aria-hidden>
                |
              ...
              {statsConfig.npmPackages.map((pkg, index) => {
                return (<Fragment key={pkg}>
                  ...
                  <dt className="sr-only">{pkg}</dt>
                  <dd className="text-zinc-500/50" title={`Last 30 days, ${pkg}`}>
                    {formatStatNumber(
