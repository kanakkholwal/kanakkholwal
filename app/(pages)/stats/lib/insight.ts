...
export const getProjectInsight = cache(async (project: ProjectConfig): Promise<InsightResponse> => {
    try {
        ...
        if (!res.ok) {
            ...
            return Promise.reject(new Error(`Failed to fetch project insight data: ${res.status} ${res.statusText}`));
        }
...
export function cumulateStats(usersStats: UserCountAndGrowthResult, sessionsStats: SessionCountAndGrowthResult) {
  ...
  const userMap = new Map(
    usersStats.graphData.map((d: any) => [(new Date(d.timestamp)).getTime(), d.count])
  )
  const sessionMap = new Map(
    sessionsStats.graphData.map((d: any) => [(new Date(d.timestamp)).getTime(), d.count])
  )
