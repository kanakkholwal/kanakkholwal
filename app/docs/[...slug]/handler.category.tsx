...
const CATEGORY_META: Record<string, { title: string; description: string }> = {
  ...
  reliability: {
    ...
    description:
      "Incident reports, chaos engineering, and failure mode analysis.",
  },
  security: {
...
export default async function CategoryPage(props: { category: string }) {
          ...
          {categoryPages.map((page) => (
            ...
            >
              ...
              <div className="text-right font-mono text-xs text-muted-foreground group-hover:text-foreground transition-colors">
                {Math.ceil((page.data.body.toString().length || 1000) / 1000)}{" "}
                min
              </div>
            </Link>
          ))}
